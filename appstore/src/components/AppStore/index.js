...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredResults = appsList.filter(
      eachrow => eachrow.category === tabId,
    )
    const finalResults = filteredResults.filter(eachrow =>
      eachrow.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="container">
        <div className="cardContainer">
          ...
          <ul className="tabsContainer">
            {tabsList.map(eachrow => (
              <TabItem
                key={eachrow.tabId}
                properties={eachrow}
                special={tabId === eachrow.tabId}
          ...
          <ul className="appsContainer">
            {finalResults.map(eachrow => (
              <AppItem properties={eachrow} key={eachrow.appId} />
            ))}
          </ul>
        </div>
      </div>
